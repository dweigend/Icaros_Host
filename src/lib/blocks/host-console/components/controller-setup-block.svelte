...
<script lang="ts">
	...
	import { Button, Kbd, ScrollArea, StatusDot, Switch } from '$lib/components';
...
<section class="card" aria-labelledby="usb-title">
	...
	<ol class="runtime-clients" aria-label="Controller setup checklist">
		{#each state.controllerIndicators as indicator (indicator.label)}
			<li class="runtime-client" data-tone={indicator.tone}>
				<div class="status">
					<StatusDot tone={indicator.tone} label={`${indicator.label} ${indicator.value}`} />
	...
	<form class="stack" method="POST" action="?/connectUsb">
		<div class="form-grid">
			<label>
				<span>SSID</span>
				<input name="ssid" bind:value={state.usbForm.ssid} autocomplete="off" />
			</label>
			<label>
			...
			<label>
				<span>Device ID</span>
				<input name="deviceId" bind:value={state.usbForm.deviceId} autocomplete="off" />
			</label>
			<label>
