...
const DEVICE_PAIRING_TOKEN_GLOBAL_KEY = Symbol.for(
  "icaros.host.devicePairingToken",
);
...
export function createDevicePairingTokenFingerprint(
...
function readSharedPairingToken(): string {
  ...
  const generatedToken =
    randomBytes(FALLBACK_TOKEN_BYTES).toString("base64url");
...
function writePersistedToken(token: string): void {
  mkdirSync(dirname(PERSISTED_PAIRING_TOKEN_FILE), {
    recursive: true,
    mode: 0o700,
  });
